...
function checkFile(file) {
  ...
  // Парсим как скрипт; для JSX/TS возможны ложные срабатывания, но esprima корректно собирает комментарии
  ...
  for (const c of parsed.comments || []) {
    ...
    if (!hasCyrillic(value)) {
      ...
      // Возьмём короткий фрагмент для сообщения
...
function main() {
  ...
  for (const f of files) {
    try {
      ...
      // Если не удалось распарсить файл — не блокируем коммит, но уведомим
